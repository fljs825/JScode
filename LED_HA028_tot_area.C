void LED_HA028_tot_area(const char* filename = "filename.root")
{
	// Load root files
	...
	TH1D *h1 = new TH1D("h1","HA028 5000 mV SPE area dist;Area (ADC*TDC);The number of events", 300, -200, 200);
    TH1D *h2 = new TH1D("h2","HA028 5000 mV with TL SPE area dist;Area (ADC*TDC);The number of events", 300, -200, 200);
    TH1D *h3 = new TH1D("h3","HA028 6000 mV SPE area dist;Area (ADC*TDC);The number of events", 300, -200, 200);
	TH1D *h4 = new TH1D("h4","HA028 6000 mV with TL SPE area dist;Area (ADC*TDC);The number of events", 300, -200, 200);
	...
	for (int i = 0; i < n1; i++)
	{
		tree1 -> GetEvent(i);
		h1 -> Fill(area1);
		if(ZCtime1 > 2000 && ZCtime1 < 2500 && pulseN1 < 3) h1 -> Fill(area1);
	}
	for (int i = 0; i < n2; i++)
	{
		tree2 -> GetEvent(i);
		h2 -> Fill(area2);
		if(ZCtime2 > 2000 && ZCtime2 < 2500 && pulseN2 < 3) h2 -> Fill(area2);
	}
	for (int i = 0; i < n3; i++)
	{
		tree3 -> GetEvent(i);
		h3 -> Fill(area3);
		if(ZCtime3 > 2000 && ZCtime3 < 2500 && pulseN3 < 3) h3 -> Fill(area3);
	}
	for (int i = 0; i < n4; i++)
	{
		tree4 -> GetEvent(i);
		h4 -> Fill(area4);
		if(ZCtime4 > 2000 && ZCtime4 < 2500 && pulseN4 < 3) h4 -> Fill(area4);
	}
